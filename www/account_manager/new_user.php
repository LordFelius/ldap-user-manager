...
if (isset($_GET['account_request'])) {
    foreach ($request_index as $attribute) {
        ...
        $label = $attr_r['label'];
        $$attribute = filter_var($_GET[$attribute], FILTER_SANITIZE_STRING);
        $new_account_r[$attribute] = $$attribute;
    }
    if (isset($email) || isset($mail)) {
...
if (isset($_POST['create_account'])) {
    ...
    $mail_attribute = $new_account_r[$LDAP['search_attribute']];
    if (isset($_POST['send_email']) and isset($mail) and $EMAIL_SENDING_ENABLED == TRUE) { $send_user_email = TRUE; }

    //I would trust the precautions are done in url generation (in request_account.php), and the Manager know what is being done.
    if (TRUE) {
        ...
        if ($new_account) {
            ...
            if (isset($send_user_email) and $send_user_email == TRUE) {
                ...
                $mail_body = parse_mail_text($new_account_mail_body, $password, $mail_attribute, "Dear", "User");
                $mail_subject = parse_mail_text($new_account_mail_subject, $password, $mail_attribute, "Dear", "User");

                $sent_email = send_email($mail,"Dear User",$mail_subject,$mail_body);
                $creation_message = "The account was created";
                if ($sent_email) {
