...
function ldap_auth_username($ldap_connection,$username, $password) {
    ...
    $ldap_search_query="${LDAP['search_attribute']}=" . ldap_escape($username, "", LDAP_ESCAPE_FILTER);
...
function ldap_get_user_list($ldap_connection,$start=0,$entries=NULL,$sort="asc",$sort_key=NULL,$filters=NULL,$fields=NULL) {
    ...
    if (!isset($fields)) { $fields = ldap_complete_initial_index(); }
    ...
    foreach ($result as $record) {
        ...
        if (isset($record[$sort_key][0])) {
            ...
            foreach($fields as $this_attr) {
                if ($this_attr !== $sort_key && isset($record[$this_attr])) { $add_these[$this_attr] = $record[$this_attr][0]; }
            }
    ...
    return(array_slice($records,$start,$entries,true));
...
//search with search_attribute; return account_attribute
function ldap_get_dn_of_user($ldap_connection,$user_name) {
    ...
    if (isset($user_name)) {
        ...
        $ldap_search = @ ldap_search($ldap_connection, "${LDAP['user_dn']}", $ldap_search_query , array("dn"));
        $result = @ ldap_get_entries($ldap_connection, $ldap_search);
...
function ldap_is_group_member($ldap_connection,$group_name,$username) {
    ...
    if ($ldap_search) {
        ...
        if ($LDAP['group_membership_uses_uid'] == FALSE) {
            $username = ldap_get_dn_of_user($ldap_connection,$username);
        }
...
function ldap_user_group_membership($ldap_connection,$username) {
    ...
    $ldap_search_query = "(&(objectClass=${LDAP['group_class']})(${LDAP['group_membership_attribute']}=${username}))";
...
function ldap_new_group($ldap_connection,$group_name,$initial_member="") {
    ...
    if (isset($group_name)) {
        ...
        if ($result['count'] == 0) {
            ...
            if ($rfc2307bis_available == FALSE) { $objectclasses = array('top',$LDAP['group_class']); } else { $objectclasses = array('top','groupOfUniqueNames','posixGroup'); }
...
function ldap_complete_account_attribute_array() {
    ...
    if (isset($LDAP['account_additional_attributes'])) {
        ...
        $attribute_r = $additional_attributes_r;
...
function ldap_complete_request_index() {

    global $LDAP;

    $attribute_r = $LDAP['default_request_index'];

    if (isset($LDAP['set_request_index'])) {
        $attribute_r = explode(",", $LDAP['set_request_index']);
    }
    return($attribute_r);

}

function ldap_complete_initial_index() {

    global $LDAP;

    $attribute_r = $LDAP['default_initial_index'];

    if (isset($LDAP['set_initial_index'])) {
        $attribute_r = explode(",", $LDAP['set_initial_index']);
    }
